...
interface AddIdpFormProps {
  t: Dictionary
  params: { lang: Locale; id: string }
}

export const AddIdpForm: React.FC<AddIdpFormProps> = ({
  params: { lang, id },
  t
}: AddIdpFormProps) => {
  ...
  async function onSubmit(formData: FormData) {
    try {
      setIsLoading(true)
      const { data } = await api.post('/idps/', { ...formData, authId: id })

      toast({
        title: 'Success',
        description: 'Individual Plan objective Added successfully'
      })
      refresh()
      form.reset()
    } catch (error: any) {
