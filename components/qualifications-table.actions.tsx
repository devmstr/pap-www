...
interface QualificationsTableActionsProps {
  id: string
}

export function QualificationsTableActions({
  id
}: QualificationsTableActionsProps) {
  ...
  const handleDelete = async (qualificationId: string) => {
    try {
      setIsDeleting(true)
      const { data } = await api.delete('/qualification/' + qualificationId)
      refresh()
    } catch (error: any) {
      toast({
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem asChild>
            <Link
              ...
              href={`/dashboard/qualifications/${id}`}
            >
