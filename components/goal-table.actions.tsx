...
interface GoalTableActionsProps {
  id: string
}

export function GoalTableActions({ id }: GoalTableActionsProps) {
  ...
  const handleDelete = async (goalId: string) => {
    try {
      setIsDeleting(true)
      const { data } = await api.delete('/performance/' + goalId)
      refresh()
    } catch (error: any) {
      toast({
          ...
          <DropdownMenuItem asChild>
            <Link
              className={cn(
                ...
                'flex gap-3 items-center justify-center w-12 cursor-pointer group ring-0  '
              )}
              href={'/dashboard/performance/' + id}
            >
